...
3. **Re-sync when the BIOS changes.** The `bios` goldens pinned here are captured from the
   ...
   checkpoint.
4. **One spec.** `SPEC.md` here duplicates `6502-EMULATOR/docs/VDP-SPEC.md`, and
   `docs/SPEC.html` is rendered from it. Agree which is the source and how the copy is
   kept in step.
5. **The detection contract.**
...
6. **A release people can install.** A versioned UF2 on GitHub releases, with flashing
...
7. **Optional bench acceptance:** BIOS 2.0 booting and scrolling on the PRO. It is the
